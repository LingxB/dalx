...
seed_df = pd.merge(_df, seed_vectors.reset_index(), how='left', on='SEED').set_index(['SEED', 'ASP', 'LABELS'])
aspect_df = pd.merge(_df, aspect_vectors.reset_index(), how='left', on='ASP').set_index(['SEED', 'ASP', 'LABELS'])

train_df_avg = ((seed_df + aspect_df) / 2)
train_df_add = (seed_df + aspect_df)

train_df_avg = train_df_avg.sample(frac=1, random_state=42)
train_df_add = train_df_add.sample(frac=1, random_state=42)


train_df_avg.to_csv('data/processed/seed_aspect_vectors_11_avg.csv')
train_df_add.to_csv('data/processed/seed_aspect_vectors_11_add.csv')
