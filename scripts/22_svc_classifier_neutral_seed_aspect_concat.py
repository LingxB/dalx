...
df = pd.read_csv('data/processed/seed_aspect_vectors_21.csv', index_col=['SEED', 'ASP'], dtype={'LABELS': str})
...
_w = w2vec_model['affordable'].reshape(1,-1)
...
for w in lx_df.index.get_level_values('WORD').unique():
    preds = score_df.loc[w].DALX.unique()
    preds = preds[~np.isnan(preds)]
    if len(preds) > 1:
        print(f'\n {w}')
        print(score_df.loc[w])

# w2vec_model.most_similar(positive=['product', 'great'], negative=['performance'], topn=20)
