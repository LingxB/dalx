...
df = pd.read_csv('data/processed/yelp_seed_vectors_19.csv', index_col='SEED', dtype={'LABELS': str})
...
# PREDICT
# -------
w2vec_model = Word2Vec.load('data/raw/yelp_w2vec/yelp_w2vec.model')

_x = w2vec_model['loud'].reshape(1,-1)
svc.predict(_x)
...
vocabs = set(w2vec_model.wv.index2entity) # 363658
score_words = [w for w in lx_words if w in vocabs] # 10016
prediction = []
...
for w in score_words:
    ...
    if conf_threshold:
        ...
        if prob.max() < conf_threshold:
            ...
            prediction.append(svc.predict(_x).astype(int)[0])
    else:
...
# diff frame
_df = score_df.loc[score_df.DALX.notna()]
_df.loc[_df.drop('DALX', axis=1).mean(axis=1) != _df.DALX]
...
score_df.to_csv('data/output/yelp_lexicon_table_dalx_20_thres0.7_C10.csv')
